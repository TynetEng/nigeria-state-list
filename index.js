...
let states =    [
    ...
    let display = document.querySelector('#dropdown');
    let display2 = document.querySelector('#dropdown2');
    function stateList() {
        for(i=0; i< states.length; i++){
            display.innerHTML += `<option>${states[i].state.name}</option>`
        }
        localGovt()
    }
    ...
    function change(event){
        display2.innerHTML = '';
        for(b=0; b<states.length; b++){
            let k = states[b].state;
            if( k.name == event){
                for(m=0; m<states.length; m++){
                    display2.innerHTML += `<option value= k.locals[m].name>${k.locals[m].name}</option>`
                }
            }
        }
    }
